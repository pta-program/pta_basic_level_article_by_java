...
static FastScanner fs = new FastScanner(System.in);
    ...
    int n; int x;
    char[] name = new char[105]; char[] id = new char[105]; char[] hi = "".toCharArray(); char[] hid = "".toCharArray(); char[] lo = "".toCharArray(); char[] loid = "".toCharArray();
    int max = - 1; int min = 101;
    n = fs.nextInt();
    while (n-- > 0) {
        name = fs.nextChars(); id = fs.nextChars(); x = fs.nextInt();
        if (x > max) {
            max = x;
            hi = name.clone();
            hid = id.clone();
