...
本题的核心是：过滤无效自评分后，对每位同学的有效互评排序并取中位数。程序先读取题目输入，再按照上述规则完成数据处理，最后严格按照题目要求输出结果。实现时应优先根据题目约束选择合适的数据类型和数据结构，避免溢出或不必要的重复计算。

时间复杂度为 O(nm log m)；空间复杂度取决于输入规模，主要用于保存题目数据和中间结果。

### 代码流程说明

1. 读取 1077 互评成绩计算 所需的输入数据。
2. 初始化计数器、数组或其他辅助变量。
3. 过滤无效自评分后，对每位同学的有效互评排序并取中位数。
4. 按题目规定的格式输出计算结果。
...
/*
 * 题目：1077 互评成绩
 * 实现原理：过滤无效自评分后，对每位同学的有效互评排序并取中位数。
 * 复杂度：O(nm log m)
 * 实现步骤：
 * 1. 读取 1077 互评成绩计算 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    for (int i = 0; i < n; i ++) {
        int teacher; int[] score = new int[100]; int c = 0;
        teacher = fs.nextInt();
        for (int j = 1; j < n; j ++) {
            x = fs.nextInt();
            if (x >= 0 && x <= m) score[c ++] = x;
        }
        int min = score[0]; int max = score[0]; int sum = 0;
        for (int j = 0; j < c; j ++) {
            if (score[j] < min) min = score[j];
            if (score[j] > max) max = score[j];
            sum += score[j];
        }
        double peer = (double) (sum - min - max) / (c - 2);
        printf("%d\n", (int) ((teacher + peer) / 2 + 0.5));
    }
