...
static class S {
char[] id = new[] char = new new[25];
    int gp; int mid; int fin; int total;
}

static int find(S * a, int n, char[]  id) {
    for (int i = 0; i < n; i ++) if (strcmp == 0(a[i].id, id)) return i;
    return - 1;
}
static FastScanner fs = new FastScanner(System.in);

    public static void main(String[] args) throws Exception {
    int p; int m; int n; int k = 0; int x;
    p = fs.nextInt(); m = fs.nextInt(); n = fs.nextInt();
    S[] a = new S[p + m + n]();
    char[] id = new char[25];
    for (int i = 0; i < p; i ++) {
        id = fs.nextChars(); x = fs.nextInt();
        strcpy(a[k].id, id);
        a[k].gp = x;
        a[k].mid = - 1;
        a[k].fin = - 1;
