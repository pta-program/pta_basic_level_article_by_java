...
时间复杂度为 $O(10^{K}\log K)$；空间复杂度取决于输入规模，主要用于保存题目数据和中间结果。
...
/*
 * 题目：1104 家谱
 * 实现原理：枚举三位数字，验证各位数字和、最大公约数及素数条件。
 * 复杂度：O(10³ log n)
 * 实现步骤：
 * 1. 读取 1104 天长地久 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    int q; int k; int m; int cas = 1;
    q = fs.nextInt();
    while (q-- > 0) {
        k = fs.nextInt(); m = fs.nextInt();
        int lo = 1;
        for (int i = 1; i < k; i ++) lo *= 10;
        int hi = lo * 10 - 1; int any = 0;
        printf("Case %d\n", cas ++);
        for (int a = lo; a <= hi; a ++) if (sum(a) == m) {
            int u = m; int v = sum(a + 1);
            while (v != 0) {
                int t = u % v;
                u = v;
                v = t;
