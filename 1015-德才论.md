...
static class S {
int id; int d; int c; int g; int cat;
}

static FastScanner fs = new FastScanner(System.in);

    public static void main(String[] args) throws Exception {
    int n; int L; int H; int k = 0;
    n = fs.nextInt(); L = fs.nextInt(); H = fs.nextInt();
    S[] a = new S[n]; for (int z = 0; z < n; z++) a[z] = new S();
    while (n-- > 0) {
        S s = new S();
        s.id = fs.nextInt(); s.d = fs.nextInt(); s.c = fs.nextInt();
        if (s.d < L || s.c < L) continue;
        s.g = s.d + s.c;
        s.cat = s.d >= H && s.c >= H ? 4 : s.d >= H ? 3 : s.d >= s.c ? 2 : 1;
        a[k ++] = s;
    }
    Arrays.sort(a, 0, k);
    printf("%d\n", k);
