...
/*
 * 题目：1062 最简分数
 * 实现原理：将两个分数转成统一分母，计算和、差、积、商并约分输出。
 * 复杂度：O(1)
 * 实现步骤：
 * 1. 读取 1062 最简分数 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    long n1; long m1; long n2; long m2; long k;
    n1 = fs.nextLong(); m1 = fs.nextLong(); n2 = fs.nextLong(); m2 = fs.nextLong(); k = fs.nextLong();
    int first = 1;
    for (long x = 1; x < k; x ++) {
        if (gcd(x, k) != 1) continue;
        if (n1 * k < x * m1 && x * m2 < n2 * k) {
            if (first == 0) putchar(' ');
            printf("%lld/%lld", x, k);
            first = 0;
        }
