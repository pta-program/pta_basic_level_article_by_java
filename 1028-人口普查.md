...
/*
 * 题目：1028 生日悖论式排序
 * 实现原理：过滤有效年龄记录，并按出生日期排序找出最年长和最年轻者。
 * 复杂度：O(n log n)
 * 实现步骤：
 * 1. 读取 1028 人口普查 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    int n; int y; int m; int d; int old = 20140906; int young = 0; int c = 0;
    char[] name = new char[10]; char[] on = new char[10]; char[] yn = new char[10]; char[] date = new char[20];
    n = fs.nextInt();
    while (n-- > 0) {
        name = fs.nextChars(); date = fs.nextChars();
        sscanf(date, "%d/%d/%d", & y, & m, & d);
        int v = y * 10000 + m * 100 + d;
        if (v >= 18140906 && v <= 20140906) {
            c ++;
            if (v < old) {
                old = v;
                strcpy(on, name);
