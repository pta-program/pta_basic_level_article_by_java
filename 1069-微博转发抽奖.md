...
/*
 * 题目：1069 微博转发抽奖
 * 实现原理：按转发序号检查抽奖间隔，跳过重复中奖者并保存中奖姓名。
 * 复杂度：O(n)
 * 实现步骤：
 * 1. 读取 1069 微博转发抽奖 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    int m; int n; int s; int count = 0;
    char name[25], seen[1005][25];
    m = fs.nextInt(); n = fs.nextInt(); s = fs.nextInt();
    for (int i = 1; i <= m; i ++) {
        name = fs.nextChars();
        if (i < s) continue;
        int used = 0;
        for (int j = 0; j < count; j ++) if (strcmp == 0(seen[j], name)) used = 1;
        if (used) {
            s ++;
            continue;
        }
