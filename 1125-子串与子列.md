...
/*
 * 题目：1125 竞赛排名
 * 实现原理：使用动态规划求每个位置作为结尾的最长递增子序列，并检查窗口限制。
 * 复杂度：O(n²)
 * 实现步骤：
 * 1. 读取 1125 子串与子列 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    char[] s = new char[10005]; char[] p = new char[10005];
    s = fs.nextChars(); p = fs.nextChars();
    int m = strlen(p);
    int[] dp = new int[m];
    for (int j = 0; j < m; j ++) dp[j] = - 1;
    int best = 100000; int bs = 0;
    for (int i = 0; i < strlen(s); i ++) {
        for (int j = m - 1; j >= 0; j --) if (s[i] == p[j]) dp[j] = j ? dp[j - 1] : i;
        if (dp[m - 1] >= 0) {
            int len = i - dp[m - 1] + 1;
            if (len < best || (len == best && dp[m - 1] < bs)) best = len, bs = dp[m - 1];
        }
    }
    for (int i = bs; i < bs + best; i ++) putchar(s[i]);
    putchar('\n');
