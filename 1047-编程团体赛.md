...
static FastScanner fs = new FastScanner(System.in);
    ...
    int n; int team; int member; int score; int[] sum = new int[1005]; int best = 0;
    n = fs.nextInt();
    while (n-- > 0) {
        team = fs.nextInt(); member = fs.nextInt(); score = fs.nextInt();
        sum[team] += score;
        if (sum[team] > sum[best]) best = team;
    }
    printf("%d %d\n", best, sum[best]);
