...
/*
 * 题目：1055 集体照
 * 实现原理：按总分排序分组，最高分居中，其余成员按左右交替规则排队。
 * 复杂度：O(n log n)
 * 实现步骤：
 * 1. 读取 1055 集体照 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    P[] a = new P[10000]; for (int z = 0; z < 10000; z++) a[z] = new P(); for (int z = 0; z < 10000; z++) a[z] = new P();
    for (int i = 0; i < n; i ++) a[i].n = fs.nextChars(); a[i].h = fs.nextInt();
    Arrays.sort(a, 0, n);
    int base = n / k; int extra = n % k; int pos = 0;
    for (int row = k; row >= 1; row --) {
        int len = base + (row == k ? extra : 0); int mid = len / 2;
        P[] out = new P[len]; for (int z = 0; z < len; z++) out[z] = new P();
        out[mid] = a[pos ++];
        int left = mid - 1; int right = mid + 1;
        for (int offset = 1; offset < len; offset ++) {
            if (offset % 2 == 1 && left >= 0) out[left --] = a[pos ++];
            else out[right ++] = a[pos ++];
        }
        for (int i = 0; i < len; i ++) printf("%s%c", out[i].n, i + 1 == len ? '\n' : ' ');
