...
/*
 * 题目：1082 射击比赛
 * 实现原理：计算每位选手坐标到原点的距离平方，维护最远和最近者。
 * 复杂度：O(n)
 * 实现步骤：
 * 1. 读取 1082 射击比赛 所需的输入数据。
 * 2. 初始化计数器、数组或其他辅助变量。
...
static FastScanner fs = new FastScanner(System.in);
    ...
    int n; int id; int x; int y; int near = - 1; int far = - 1; int mn = INT_MAX; int mx = - 1;
    n = fs.nextInt();
    while (n-- > 0) {
        id = fs.nextInt(); x = fs.nextInt(); y = fs.nextInt();
        int d = x * x + y * y;
        if (d < mn) mn = d, near = id;
        if (d > mx) mx = d, far = id;
    }
    printf("%04d %04d\n", near, far);
