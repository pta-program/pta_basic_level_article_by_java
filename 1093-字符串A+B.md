...
static FastScanner fs = new FastScanner(System.in);
    ...
    char[] a = new char[1000005]; char[] b = new char[1000005];
    int[] seen = new int[127];
    /* line input handled by token scanner */
    /* line input handled by token scanner */
    for (int i = 0; i < strlen(a); i ++) if (a[i] >= ' ' && a[i] <= '~') seen[(char) a[i]] = 1;
    for (int i = 0; i < strlen(b); i ++) if (b[i] >= ' ' && b[i] <= '~') seen[(char) b[i]] = 1;
    for (int pass = 0; pass < 2; pass ++) {
        char[]  s = pass ? b : a;
        for (int i = 0; i < strlen(s); i ++) if (s[i] >= ' ' && s[i] <= '~' && seen[(char) s[i]]) putchar(s[i]), seen[(char) s[i]] = 0;
    }
    putchar('\n');
